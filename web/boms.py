...
class BomsHandler(BaseHandler):
    def get(self):
        ...
        if q:
            qry = models.Bom.query(ndb.OR(models.Bom.public_id == q,
                                          models.Bom.name == q,
                                          models.Bom.tag_name == q))
        else:
            qry = models.Bom.query()

        PAGE_SIZE = 5
        if forward:
            boms, next_cursor, more = qry.order(models.Bom.key).fetch_page(PAGE_SIZE, start_cursor=cursor)
